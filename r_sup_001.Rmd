...
head(df)
```
### A randon forest dession tree method
...
logK_prediction <- predict(logK.rf, train)
train["logKcalc"] <- as.data.frame(logK_predictions)
...
```{r prediction of logK for training set}
logK_prediction_test <- predict(logK.rf, test)
test["logKcalc"] <- as.data.frame(logK_prediction_test)
```
...
paste("rmse = ", rmse_value)
```
#### A Multilayer Perceptron nureal netowrks method
